...
import { ShoppingCart, User, Globe, Sun, Moon, Menu, X, LogOut, LayoutGrid, Maximize, Minimize } from 'lucide-react';
      ...
      <div className="mx-auto flex h-14 max-w-7xl items-center justify-between px-3 sm:h-16 sm:px-4 md:h-[4.5rem]">
        ...
        <Link to="/" className="flex items-center gap-2 sm:gap-3">
          <img
            src="/logo.png?v=2"
            alt={t('siteName')}
            className="h-8 w-8 object-contain sm:h-10 sm:w-10 md:h-11 md:w-11 lg:h-12 lg:w-12"
          />
        ...
        <div className="flex items-center gap-2">
          ...
          {user ? (
            <div className="flex items-center gap-1">
              <Link to="/account" className="hidden rounded-lg px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-800 sm:flex items-center gap-1">
            ...
            <Link to="/auth/login" className="btn-primary text-xs px-3 py-2">
              {t('login')}
            </Link>
          )}
